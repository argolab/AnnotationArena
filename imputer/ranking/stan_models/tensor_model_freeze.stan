/*
 * Round 2 variant of tensor_domain_model.stan for two-round inference.
 *
 * In Round 1 (tensor_domain_model.stan), all parameters are sampled on
 ...
 *
 * In Round 2 (this file), only e_loadings are free parameters.
 * Observed ratings are test-instance only, so e_loadings[1..K_train]
 * receive no gradient signal and remain near their initialisation;
 * only e_loadings[K_train+1..K] are meaningfully updated.
 *
 * Everything else — functions, transformed parameters, model structure,
 * generated quantities — is identical to tensor_domain_model.stan.
 */
...
data {
    ...
    array[N_pairwise_rankings, 2] int<lower=1, upper=K> pairwise_ranking_items;
    array[N_pairwise_rankings] int<lower=1, upper=2> pairwise_ranking_orders;

    // Missing variables to predict
    ...
    matrix[I, D]        v_loadings;    // attribute loadings  — fixed from Round 1
    matrix[J, D]        u_loadings;    // annotator loadings  — fixed from Round 1
    array[J] simplex[C] rating_probs;  // rating probabilities — fixed from Round 1
}
...
parameters {
    // Only item loadings are free in Round 2.
    // e_loadings[1..K_train] receive no gradient signal (no train ratings present);
    // only e_loadings[K_train+1..K] are meaningfully updated.
...
model {
    ...
    // ===== PRIOR on e_loadings only =====
    // (v_loadings, u_loadings, rating_probs are fixed data)
    for (k in 1:K)
        for (d in 1:D)
    ...
    // ===== PAIRWISE RANKING LIKELIHOOD (Bradley-Terry) =====
...
generated quantities {
    ...
    for (n in 1:N_pairwise_rankings) {
        ...
        if (DEBUG_PRINT == 1 && n <= 10)
            print("[DEBUG PAIR] n=", n, " logit=", score1 - score2);
    }
    ...
    // ===== POSTERIOR PREDICTIVE FOR MISSING PAIRWISE =====
    for (n in 1:N_missing_pairwise_rankings) {
        ...
        missing_pairwise_ranking_predictions[n] =
            (score1 + gumbel1 > score2 + gumbel2) ? 1 : 2;

        if (DEBUG_PRINT == 1 && n <= 10)
            print("[DEBUG MPAIR] n=", n,
                  " logit=", score1 - score2,
                  " sampled=", missing_pairwise_ranking_predictions[n]);
    }
}