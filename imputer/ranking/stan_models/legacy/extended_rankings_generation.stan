...
generated quantities {
    ...
    // ===== GENERATE PAIRWISE RANKINGS FOR ALL OBSERVED ITEMS (EXTENDED PROTOCOL) =====
    ...
    for (i in 1:I) {
        for (j in train_annotator_start:train_annotator_end) {
            ...
            // Find ALL observed items for this (i,j) - not just tied items
            array[K_train] int observed_items;
            int num_observed = 0;

            for (k in 1:K_train) {
                if (train_rating_observed[ij_idx, k] == 1) {
                    num_observed += 1;
                    observed_items[num_observed] = k;
                }
            }

            // Generate per-item capped comparisons (sample without replacement)
            if (num_observed >= 2 && pairwise_cap_per_item > 0) {
                for (idx1 in 1:num_observed) {
                    int item1 = observed_items[idx1];
                    int rating1 = train_rating_values[ij_idx, item1];

                    // Create list of available comparison partners (excluding idx1)
                    array[num_observed-1] int available_indices;
                    int num_available = 0;
                    for (idx in 1:num_observed) {
                        if (idx != idx1) {
                            num_available += 1;
                    ...
                    for (comp_idx in 1:max_comparisons) {
                        ...
                        int item2 = observed_items[idx2];
                        int rating2 = train_rating_values[ij_idx, item2];
                        ...
                        // Store the rating value of item1 (for compatibility, though not "tied" anymore)
                        int stored_rating = rating1;
                        ...
                        train_pairwise_tied_rating[num_train_pairwise_rankings] = stored_rating;
        ...
        for (i in 1:I) {
            for (j in test_annotator_start:test_annotator_end) {
                ...
                // Find ALL observed items for this (i,j) - not just tied items
                array[K_test] int observed_items;
                int num_observed = 0;
                ...
                if (num_observed >= 2 && pairwise_cap_per_item > 0) {
                    for (idx1 in 1:num_observed) {
                        int item1 = observed_items[idx1];
                        int rating1 = test_rating_values[ij_idx, item1];
                        ...
                        for (comp_idx in 1:max_comparisons) {
                            ...
                            int idx2 = available_indices[sample_idx];
                            int item2 = observed_items[idx2];
                            int rating2 = test_rating_values[ij_idx, item2];
                            ...
                            test_pairwise_tied_rating[num_test_pairwise_rankings] = stored_rating;
