...
# Variant with --use-random-as-key enabled, MARFORMER_FULL_RANDOM=1 (fresh randomness)
...
MARFORMER_FRESH_RANDOM_BATCH_SIZE=10  # Batch size for fresh randomness (1=no batching)
...
RUN_NAME="marformer_random_fresh_lightning_4_large_network"  # Leave empty for auto-generated name, or specify custom name
...
echo "  Fresh random batch size: $MARFORMER_FRESH_RANDOM_BATCH_SIZE"
echo "  Gradient clipping: $MARFORMER_GRADIENT_CLIP_VAL"
echo "  Cosine schedule: $MARFORMER_USE_COSINE_SCHEDULE (warmup steps: $MARFORMER_WARMUP_STEPS)"
echo "  Use random as key: enabled (pointer behavior)"
echo ""
echo "Stan settings:"
...
MARFORMER_ARGS="--data-dir OUTPUT/generated_data/${RUN_NAME} \
    ...
    --save-model-every 5 \
    --fresh-random-batch-size $MARFORMER_FRESH_RANDOM_BATCH_SIZE \
...
echo "  - TensorBoard logs: OUTPUT/IMPUTER/${RUN_NAME}_marformer/lightning_logs/"
...
echo "  tensorboard --logdir OUTPUT/IMPUTER/${RUN_NAME}_marformer/lightning_logs/"
echo "=============================================="
