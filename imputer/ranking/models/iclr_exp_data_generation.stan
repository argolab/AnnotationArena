...
generated quantities {
...

    {
        ...
        if (hold_I_constant == 1) {
            ...
            for (d in 1:D) {
                global_mean_pref[d] = exponential_rng(1.0);
            }
            for (i in 1:I) {
            ...
            for (i in 1:I) {
                for (d in 1:D) {
                    mean_preferences[i, d] = exponential_rng(1.0);
                }
            }
        ...
        if (use_factored_annotator == 1) {
            ...
            if (hold_J_constant == 1) {
                ...
                // Standard case: u_j ~ Exp(1)
                for (j in 1:J) {
                    for (d in 1:d_annotator) {
                        annotator_embeddings[j, d] = exponential_rng(1.0);
                    }
                }
            }
            ...
            if (hold_I_constant == 1) {
                ...
                for (r in 1:d_annotator) {
                    for (c in 1:D) {
                        shared_M[r, c] = exponential_rng(1.0) * sigma_M;
                    }
                }
                for (i in 1:I) {
                ...
                for (i in 1:I) {
                    for (r in 1:d_annotator) {
                        for (c in 1:D) {
                            attr_transforms[i, r, c] = exponential_rng(1.0) * sigma_M;
                        }
                    }
                }
            }
            ...
            if (hold_I_constant == 1 && hold_J_constant == 1) {
                ...
                for (d in 1:D) {
                    shared_pref[d] = global_mean_pref[d] + exponential_rng(1.0) * sigma_annotator;
                }
                for (i in 1:I) {
                    for (j in 1:J) {
                        int idx = (i-1)*J + j;
                ...
                for (j in 1:J) {
                    for (d in 1:D) {
                        pref_by_j[j, d] = global_mean_pref[d] + exponential_rng(1.0) * sigma_annotator;
                    }
                }
                for (i in 1:I) {
                    for (j in 1:J) {
                ...
                for (i in 1:I) {
                    for (d in 1:D) {
                        pref_by_i[i, d] = mean_preferences[i, d] + exponential_rng(1.0) * sigma_annotator;
                    }
                }
                for (i in 1:I) {
                    for (j in 1:J) {
                ...
                for (i in 1:I) {
                    for (j in 1:J) {
                        int idx = (i-1)*J + j;
                        for (d in 1:D) {
                            annotator_preferences[idx, d] = mean_preferences[i, d] + exponential_rng(1.0) * sigma_annotator;
                        }
                    }
                }
            }
    ...
    // These are used when derive_thresholds_from_annotator=1
    {
        ...
        for (c in 1:(C-1)) {
            for (d in 1:d_annotator) {
                threshold_transform_W[c, d] = exponential_rng(1.0) * sigma_W;
            }
        }
        ...
        for (i in 1:I) {
            for (c in 1:(C-1)) {
                threshold_attr_bias[i, c] = exponential_rng(1.0) * sigma_bias;
            }
        }
    }
    ...
    // Generate training item embeddings: e_k_train ~ Exp(1)
    if (hold_K_constant == 1) {
        // Single shared item embedding across all training items (no K-dependence)
        {
            ...
            for (d in 1:D) {
                shared_train_embedding[d] = exponential_rng(1.0);
            }
            for (k in 1:K_train) {
    ...
    // Generate test item embeddings: e_k_test ~ Exp(1) - DIFFERENT from training
    if (hold_K_constant == 1) {
        // Single shared item embedding across all test items (no K-dependence)
        {
            row_vector[D] shared_test_embedding;
            for (d in 1:D) {
                shared_test_embedding[d] = exponential_rng(1.0);
            }
            for (k in 1:K_test) {
