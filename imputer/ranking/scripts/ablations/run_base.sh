...
# ── Ablation Base ─────────────────────────────────────────────────────────────
# Best run config (shared-bias, scale_shared_rel, item+attr L2 reg, kaiming,
# llm-input-dist) with pointer / add-one-attn / rel-value all removed.
# All other ablation scripts add exactly one (or all) of these back.
...
RUN_NAME="ablation_base_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
...
RUN_NAME="${RUN_NAME}_sharedbias_relscale_softinput_${BUNDLE}"

echo ""
echo "============================================================"
echo " Ablation Base (no pointer / no add-one / no rel-value)"
