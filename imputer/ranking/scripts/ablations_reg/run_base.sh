...
# ── Ablation 3: BASE ──────────────────────────────────────────────────────────
# 8L noperhead, kaiming, no reg, no deviation norm, constant LR 2e-4.
# Identical to ablations_more/run_base.sh — serves as the anchor for this set.
...
RUN_NAME="ablation3_base_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
