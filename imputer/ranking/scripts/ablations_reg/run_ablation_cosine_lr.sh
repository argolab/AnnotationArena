...
# ── Ablation 3: COSINE LR SCHEDULE ───────────────────────────────────────────
# Cosine annealing from 2e-3 (fast early convergence) down to 1e-5 over 300 epochs.
# Other settings identical to base (no reg, no deviation norm).
...
RUN_NAME="ablation3_cosine_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
...
echo ""
echo "============================================================"
echo " Ablation 3: COSINE LR SCHEDULE  (${LR} -> ${LR_MIN})"
