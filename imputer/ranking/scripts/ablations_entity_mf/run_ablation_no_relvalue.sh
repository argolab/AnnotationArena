...
#SBATCH --job-name=EntityMF_NoRelV
#SBATCH --nodes=1
...
# ── Ablation: NO RELATIONAL VALUE AUGMENTATION ───────────────────────────────
# Removes the per-relation value correction V_{ij} = V(x_j) + sum_r e_r * edge[i,j,r].
# Value aggregation is standard (relation-agnostic). Everything else identical to base.
...
USE_POINTER=true
USE_REL_VALUE=false       # <-- ablated: no relation-specific value augmentation
...
RUN_NAME="ablation_norelv_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ff${D_FF}_${EPOCHS}ep"
...
echo ""
echo "============================================================"
echo " Ablation: NO RELATIONAL VALUE AUGMENTATION"
echo "  bundle:       $BUNDLE"
