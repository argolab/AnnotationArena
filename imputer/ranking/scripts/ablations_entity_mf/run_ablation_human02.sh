...
#SBATCH --job-name=EntityMF_Human02
#SBATCH --nodes=1
...
# ── Ablation: HUMAN OBSERVED RATE = 0.2 ──────────────────────────────────────
# 20% of human annotations are kept observed during training (structured masking).
# Tests whether a small fraction of observed human labels improves imputation.
...
LLM_ANNOTATOR_ID=24
HUMAN_OBSERVED_RATE=0.2   # <-- ablated: 20% of human annotations kept observed
MAX_ITEM=10
...
FLAGS="${FLAGS}_human${HUMAN_OBSERVED_RATE}"

RUN_NAME="ablation_human02_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ff${D_FF}_${EPOCHS}ep"
...
echo ""
echo "============================================================"
echo " Ablation: HUMAN OBSERVED RATE = 0.2"
echo "  bundle:       $BUNDLE"
