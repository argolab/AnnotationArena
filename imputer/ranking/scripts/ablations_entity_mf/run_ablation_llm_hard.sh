...
#SBATCH --job-name=EntityMF_LLMHard
#SBATCH --nodes=1
...
# ── Ablation: LLM HARD LABELS ─────────────────────────────────────────────────
# Uses the hard-label bundle (one-hot spike encoding for LLM ratings) instead of
# soft distribution encoding. BUNDLE=hard, llm-input-dist disabled.
# Everything else identical to base case.

# ── Bundle variant ────────────────────────────────────────────────────────────
BUNDLE="hard"                                    # <-- ablated: hard-label bundle
DATA_DIR="OUTPUT/generated_data/llm_rubric"     # <-- ablated: hard bundle path
...
USE_LLM_INPUT_DIST=false  # <-- ablated: hard one-hot spike instead of log-prob dist
...
RUN_NAME="ablation_llmhard_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ff${D_FF}_${EPOCHS}ep"
