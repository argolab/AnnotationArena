...
#SBATCH --job-name=EntityMF_NoPtr
#SBATCH --nodes=1
...
# ── Ablation: NO POINTER (K_AUG) ─────────────────────────────────────────────
# Removes direct obs-obs shared-identity bias. The model must rely on 2-hop
# paths through entity tokens to connect observations sharing I/J/K.
...
USE_POINTER=false         # <-- ablated: no K_aug pointer
USE_REL_VALUE=true
...
RUN_NAME="ablation_noptr_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ff${D_FF}_${EPOCHS}ep"
...
echo ""
echo "============================================================"
echo " Ablation: NO POINTER (K_AUG)"
echo "  bundle:       $BUNDLE"
