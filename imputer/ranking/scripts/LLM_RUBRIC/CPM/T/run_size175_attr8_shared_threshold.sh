...
#SBATCH --job-name=CPM_LLM_RUBRIC_LLMRubric_225_25_8_175_ST
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
SPLIT="LLMRubric_225_25_8_175"
DATA_DIR="DATA/LLM_RUBRIC/${SPLIT}"
DATA_BUNDLE="${DATA_DIR}/data_bundle.json"
OUTPUT_DIR="RESULTS/STAN/LLM_RUBRIC/CPM"
RUN_NAME="LLMRubric_225_25_8_175_shared_threshold"
STAN_FILE="STAN/stan_models/shared_threshold_tensor_model.stan"
...
TMP_CONFIG=$(mktemp /tmp/cpm_shared_threshold_config.XXXXXX.json)
trap 'rm -f "$TMP_CONFIG"' EXIT
write_temp_tensor_config "${DATA_DIR}/configs.json" "$TMP_CONFIG"

echo ""
echo "============================================================"
echo " LLM RUBRIC | Shared-Threshold Tensor | ${SPLIT}"
echo "  RUN_NAME      : ${RUN_NAME}"
echo "  STAN_FILE     : ${STAN_FILE}"
echo "  CHAINS        : ${CHAINS}"
echo "  ITER_WARMUP   : ${ITER_WARMUP}"
echo "  ITER_SAMPLING : ${ITER_SAMPLING}"
echo "============================================================"

echo "[1/2] Running MCMC (shared-threshold tensor)..."
