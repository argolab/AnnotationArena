...
# ── Hyperparam search: 5 attention heads (vs base 4) ─────────────────────────
# head_dim = 80 / 5 = 16  (divisible ✓)
...
EMBEDDING_DIM=80
NUM_LAYERS=8
ATTENTION_HEADS=5      # <-- ablated: 5 vs base 4  (head_dim = 16)
D_FF=128
NUM_FFN_LAYERS=1
DROPOUT=0.1
...
RUN_NAME="multirun_5heads_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
...
echo ""
echo "============================================================"
echo " Hyperparam search: 5 attention heads (head_dim=16)"
