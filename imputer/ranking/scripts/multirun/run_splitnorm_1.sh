...
#SBATCH --job-name=EMF_SplitNorm_R1
#SBATCH --nodes=1
...
# ── Split-Stream Norm Run 1 ───────────────────────────────────────────────────
# Same as base config but with --use-split-stream-norm enabled.
# 4 independent LayerNorms per block (feat_attn, param_attn, feat_ff, param_ff)
# instead of 2 shared norms. Fixes scale asymmetry between feature (~1-2) and
# param (~20x) streams.
