...
# ── Base Run 1 (no split-stream norm) ────────────────────────────────────────
# Best performing config: shared-bias (no-per-head-rel) + pointer + rel-value
# + add-one-attn + soft-input + both L2 reg (item+attr) + scale_shared_rel.
# 3 independent seeds to estimate variance.
...
RUN_NAME="multirun_base_run${RUN_NUMBER}_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
...
echo ""
echo "============================================================"
echo " Multirun Base Run ${RUN_NUMBER} (no split-stream norm)"
...
echo "  init:         ${TYPE_EMBEDDING_INIT}  scale_shared_rel=true  split_stream_norm=false"
