...
#SBATCH --job-name=EMF_S2_AO_IR_DN
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
#SBATCH --qos=qos_gpu
...
# ── Section 2: EF1 (Pointer) + Add-One Attn + Item Reg + Deviation Norm ──────
# EF1 = pointer-enabled base (shared-bias + scale, item_dropout=0.7).
# Full combination: add-one attention + item L2 reg (1e-3) + deviation norm.

DATA_DIR="OUTPUT/generated_data/llm_rubric_dist"
OUTPUT_ROOT="OUTPUT/ENTITY_MF/SEC2_EXPS"
BUNDLE="dist"
...
# ── Run name base ─────────────────────────────────────────────────────────────
EXP_LABEL="ef1_addone_itemreg_devnorm"
RUN_BASE="sec2_${EXP_LABEL}_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_${EPOCHS}ep"
RUN_BASE="${RUN_BASE}_itemdrop${ITEM_DROPOUT_RATE}_ireg${ITEM_REG_WEIGHT}_areg${ATTRIBUTE_REG_WEIGHT}_${BUNDLE}"

echo ""
echo "============================================================"
echo " Sec 2: EF1 + Add-One Attn + Item Reg + Deviation Norm (3 runs)"
