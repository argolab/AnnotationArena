#!/bin/bash
#
# MARFORMER: tensor bundle Tensor_300_50_50_9_25_D16__cheating_oracle (oracle concat+freeze, D=16).
# For Tensor_400 transductive / no-item-dev (non-oracle) runs, see run_marformer_tensor400_trans_noitemdev.sh
#
# Edit DATA_ROOT if you used a different --run-name / suffix when generating data.
#
# sbatch_adapt prepends Slurm/conda; do not add #SBATCH here (see run_marformer_tensor125_oracle_500ep.sh header).
#
# Submit (script path only):
#   /home/xwang397/bin/sbatch_adapt scripts/STAN_sparse/run_marformer_tensor_300_50_50_D16_oracle_500ep.sh
#
# Interactive:
#   bash scripts/STAN_sparse/run_marformer_tensor_300_50_50_D16_oracle_500ep.sh
...
# Must match generate_data_tensor_300_50_50_D16_mcar_cheating_oracle.sh (RUN_NAME + RUN_SUFFIX)
DATA_ROOT="DATA/STAN/SPARSE/Tensor_300_50_50_9_25_D16__cheating_oracle"
OUTPUT_ROOT="RESULTS/MARFORMER/STAN_oracle_diag"
RUN_NAME="Tensor_300_50_50_9_25_D16_oracle_500ep"
...
if [[ ! -f "${BUNDLE}" ]]; then
  ...
  echo "Generate data first: bash scripts/STAN_sparse/generate_data_tensor_300_50_50_D16_mcar_cheating_oracle.sh"
  exit 1
fi
python - <<PY
import json
...
if not eff:
    raise SystemExit(
        "data_bundle.json has no eff_pref. Regenerate with current generate_data.py "
        "(tensor extraction saves eff_pref)."
    )
if "embeddings" not in d:
    raise SystemExit("data_bundle.json missing embeddings")
print("OK: eff_pref and embeddings present (oracle D from configs.json datagen.D)")
PY
...
echo ""
echo "============================================================"
echo " MARFORMER | Tensor_300_50_50 D16 oracle | ${EPOCHS} epochs"
