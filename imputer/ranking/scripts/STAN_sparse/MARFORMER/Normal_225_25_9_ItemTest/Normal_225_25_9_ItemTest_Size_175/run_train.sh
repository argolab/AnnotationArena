...
# Apache 2.0.
#
# STAN_sparse synthetic bundle — single split (see DATA/STAN_sparse/<family>/<split>/).
# Cluster: submit with sbatch_adapt from any cwd; cd to imputer/ranking first.
#
# Example (cwd must be imputer/ranking — not the AA_new repo root):
...
#     /home/xwang397/bin/sbatch_adapt scripts/STAN_sparse/MARFORMER/<family>/<split>/run_train.sh

_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
_LEAF="$(basename "${_SCRIPT_DIR}")"
_PARENT="$(basename "$(dirname "${_SCRIPT_DIR}")")"
# .../scripts/STAN_sparse/MARFORMER/<parent>/<leaf>/run_train.sh → ranking root is 5 levels up.
...
echo ""
echo "============================================================"
echo " STAN_sparse | Marformer | ${_PARENT} / ${_LEAF}"
echo "  DATA_DIR     : ${DATA_ROOT}"
echo "  OUTPUT_ROOT  : ${OUTPUT_ROOT}"
echo "  RUN_NAME     : ${RUN_NAME}"
echo "  USE_GRAPH_MASK : false"
echo "============================================================"
