#!/bin/bash
#
# Transductive MARFORMER: no item deviation (item-dropout 1.0), no LLM-input dist.
# Default data layout matches generate_tensor400_25_9_itemtest_splits.sh.
#
# Usage (from imputer/ranking):
#   bash scripts/STAN_sparse/run_marformer_tensor400_trans_noitemdev.sh [LEAF] [BUNDLE_SUFFIX]
# Defaults:
#   LEAF=Tensor_400_25_9_ItemTest_200
#   BUNDLE_SUFFIX=__cheating_oracle   (empty second arg or DATA_BUNDLE_SUFFIX= for no suffix)
#
# Examples:
#   bash scripts/STAN_sparse/run_marformer_tensor400_trans_noitemdev.sh
#   DATA_BUNDLE_SUFFIX= bash .../run_marformer_tensor400_trans_noitemdev.sh Tensor_400_25_9_ItemTest_200 ""
...
LEAF="${1:-Tensor_400_25_9_ItemTest_200}"
# Second arg overrides suffix; else env DATA_BUNDLE_SUFFIX; default __cheating_oracle to match generator
if [[ $# -ge 2 ]]; then
  BUNDLE_SUFFIX="${2}"
else
  BUNDLE_SUFFIX="${DATA_BUNDLE_SUFFIX:-__cheating_oracle}"
fi

# ── Paths ─────────────────────────────────────────────────────────────────────
DATA_ROOT="DATA/STAN/SPARSE/Tensor_400_25_9_ItemTest/${LEAF}${BUNDLE_SUFFIX}"
OUTPUT_ROOT="RESULTS/MARFORMER/STAN/SPARSE"
RUN_NAME="${LEAF}_NOITEMDEV_TRANS_MARFORMER"
...
if [[ ! -f "${BUNDLE}" ]]; then
  ...
  echo "Generate data first: bash scripts/STAN_sparse/generate_tensor400_25_9_itemtest_splits.sh"
  exit 1
fi

echo ""
echo "============================================================"
echo " CLUSTER | No Item Dev Transductive | ${LEAF}"
