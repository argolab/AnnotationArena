...
#SBATCH --job-name=EMF2_NormalInit
#SBATCH --nodes=1
...
# ── Ablation 2: NORMAL INIT (BERT-style) ─────────────────────────────────────
# Base: kaiming. BERT-style N(0, 0.02) init for type centroid embeddings.
...
# ── Initialization ────────────────────────────────────────────────────────────
TYPE_EMBEDDING_INIT="normal"     # <-- ablated: normal (base = kaiming)
...
RUN_NAME="ablation_normalinit_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ff${D_FF}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
...
echo ""
echo "============================================================"
echo " Ablation 2: NORMAL INIT (BERT-style, std=0.02)"
echo "  bundle:       $BUNDLE"
...
echo "  init:         ${TYPE_EMBEDDING_INIT}  (N(0, 0.02))"
