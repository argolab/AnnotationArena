...
# ── Ablation 2: BASE CASE ─────────────────────────────────────────────────────
# 8L, No Per-Head Rel, Pointer + Rel Value, LLM Dist, Kaiming init, Human 0.0.
...
RUN_NAME="ablation_base_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ff${D_FF}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
