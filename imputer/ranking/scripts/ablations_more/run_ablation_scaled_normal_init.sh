...
#SBATCH --job-name=EMF2_ScaledInit
#SBATCH --nodes=1
...
# ── Ablation 2: SCALED NORMAL INIT ───────────────────────────────────────────
# Base: kaiming. Scaled N(0, 1/sqrt(feature_dim)) init for type centroid embeddings.
...
# ── Initialization ────────────────────────────────────────────────────────────
TYPE_EMBEDDING_INIT="scaled_normal"  # <-- ablated: scaled_normal (base = kaiming)
...
RUN_NAME="ablation_scalednormal_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ff${D_FF}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
RUN_NAME="${RUN_NAME}_msk${MASKING_RATE}_aug${MASK_AUGMENTATIONS}_wt${MASKED_LOSS_WEIGHT}obs${OBSERVED_LOSS_WEIGHT}"
RUN_NAME="${RUN_NAME}_itemdrop${ITEM_DROPOUT_RATE}_drop${DROPOUT}_lr${LR}_init${TYPE_EMBEDDING_INIT}"
RUN_NAME="${RUN_NAME}_${FLAGS}_${BUNDLE}"

echo ""
echo "============================================================"
echo " Ablation 2: SCALED NORMAL INIT (std=1/sqrt(feature_dim))"
echo "  bundle:       $BUNDLE"
...
echo "  init:         ${TYPE_EMBEDDING_INIT}  (N(0, 1/sqrt(feature_dim)))"
