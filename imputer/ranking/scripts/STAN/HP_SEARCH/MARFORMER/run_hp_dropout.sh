...
#SBATCH --job-name=EMF_STAN_HP_DROP
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# ── SET THIS after inspecting run_hp_lr.sh val results ───────────────────────
BEST_LR=2e-4
...
echo ""
echo "============================================================"
echo " STAN HP Search | Dropout/Reg Sweep | Marformer"
echo " Data    : ${DATA_ROOT}"
echo " Output  : ${OUTPUT_ROOT}"
echo " Epochs  : ${EPOCHS}  LR: ${BEST_LR}"
echo "============================================================"

_run() {
    local RUN_NAME="$1"
    local ITEM_DROPOUT_RATE="$2"
    local ANNOTATOR_DROPOUT_RATE="$3"
    local ANNOTATOR_REG_WEIGHT="$4"
    local RUN_START=$SECONDS

    echo ""; echo "--- ${RUN_NAME} ---"
    echo "    item_dropout=${ITEM_DROPOUT_RATE}  ann_dropout=${ANNOTATOR_DROPOUT_RATE}  ann_reg=${ANNOTATOR_REG_WEIGHT}"; echo ""
...
# Run 1: annotator dropout 0.7, no reg
_run "${SPLIT}_anndrop0.7"          0.0  0.7  0.0

# Run 2: no dropout
_run "${SPLIT}_nodrop"              0.0  0.0  0.0

# Run 3: annotator dropout 0.5 + annotator reg 1e-3
_run "${SPLIT}_anndrop0.5_annreg1e-3"  0.0  0.5  1e-3

TOTAL_ELAPSED=$(( SECONDS - SCRIPT_START ))
echo ""
echo "============================================================"
echo " Dropout sweep done. Total: $(( TOTAL_ELAPSED / 60 ))m $(( TOTAL_ELAPSED % 60 ))s"
