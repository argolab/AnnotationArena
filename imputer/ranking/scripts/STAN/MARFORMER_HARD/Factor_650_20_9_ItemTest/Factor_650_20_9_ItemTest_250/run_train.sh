...
# Apache 2.0.
#
# STAN synthetic bundle — single split (see DATA/STAN/<family>/<split>/).
...
#     /home/xwang397/bin/sbatch_adapt scripts/STAN/MARFORMER_HARD/<family>/<split>/run_train.sh

_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
_LEAF="$(basename "${_SCRIPT_DIR}")"
_PARENT="$(basename "$(dirname "${_SCRIPT_DIR}")")"
# .../scripts/STAN/MARFORMER_HARD/<parent>/<leaf>/run_train.sh → ranking root is 5 levels up.
...
echo ""
echo "============================================================"
echo " STAN | Marformer Hard (graph mask) | ${_PARENT} / ${_LEAF}"
echo "  DATA_DIR     : ${DATA_ROOT}"
echo "  OUTPUT_ROOT  : ${OUTPUT_ROOT}"
echo "  RUN_NAME     : ${RUN_NAME}"
echo "  USE_GRAPH_MASK : true (hard attention / graph mask)"
echo "============================================================"
