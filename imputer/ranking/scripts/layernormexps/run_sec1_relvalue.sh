...
# ── Section 1: Base + Rel Value with Feature-Only LayerNorm ──────────────────
# Same config as final_ablations/run_exp02_relvalue.sh (shared-bias + scale,
# rel-value enabled, item_dropout=0.7) but with --use-feature-only-norm.
# NOTE: rel-value here uses the shared formulation (value_rel [R, D], mean attn
# aggregation) matching the colleague's implementation — NOT the per-head version.
# Comparison baseline: exp02 rel-value used joint LayerNorm on concat(features, params).
...
echo ""
echo "============================================================"
echo " Sec 1 LayerNorm Exp: Base + Rel Value + Feature-Only Norm (2 runs)"
