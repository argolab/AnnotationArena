...
# ── Section 1: Base + Pointer with Feature-Only LayerNorm ────────────────────
# Same config as final_ablations/run_exp01_pointer.sh (shared-bias + scale,
# pointer enabled, item_dropout=0.7) but with --use-feature-only-norm.
# Comparison baseline: exp01 pointer used joint LayerNorm on concat(features, params).
