...
# ── Section 1: Base with Feature-Only LayerNorm ───────────────────────────────
# Same config as final_ablations/run_exp00_base.sh (shared-bias + scale,
# no pointer, no rel-value, no reg, no item dropout) but with --use-feature-only-norm.
# LayerNorm applied to feature stream only (75-dim); raw params concatenated after.
# Comparison baseline: exp00 base used joint LayerNorm on concat(features, params).
...
echo ""
echo "============================================================"
echo " Sec 1 LayerNorm Exp: Base + Feature-Only Norm (2 runs)"
