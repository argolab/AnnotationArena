...
#SBATCH --job-name=EMF_Sweep_2FFN
#SBATCH --nodes=1
...
# ── Sweep: FFN LAYERS = 2 (No Per-Head Rel base) ─────────────────────────────
# Base: 1 FFN layer. This run uses 2 MLP layers inside each FFN block, all else unchanged.
...
ATTENTION_HEADS=4
D_FF=128
NUM_FFN_LAYERS=2          # <-- swept: 2 FFN layers (base = 1)
DROPOUT=0.1
