...
#SBATCH --job-name=EMF_Sweep_Drop0p3
#SBATCH --nodes=1
...
# ── Sweep: ITEM DROPOUT = 0.3 (No Per-Head Rel base) ─────────────────────────
# Base: 0.7. Lower dropout means item deviations are more often preserved during
# training — the model gets more item-specific signal but may overfit to train items.
...
NUM_FFN_LAYERS=1
DROPOUT=0.1
ITEM_DROPOUT_RATE=0.3     # <-- swept: 0.3 (base = 0.7)
