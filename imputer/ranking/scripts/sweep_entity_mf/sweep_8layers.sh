...
# ── Sweep: ENCODER LAYERS = 8 (No Per-Head Rel base) ─────────────────────────
# Base: 6L. This run increases depth to 8 layers, all else unchanged.
...
EMBEDDING_DIM=80
NUM_LAYERS=8              # <-- swept: 8 layers (base = 6)
ATTENTION_HEADS=4
D_FF=128
NUM_FFN_LAYERS=1
DROPOUT=0.1
