...
#SBATCH --job-name=EMF4_CosineBothReg
#SBATCH --nodes=1
...
# ── Ablation 4: COSINE LR + BOTH ITEM + ATTR L2 REG ─────────────────────────
# Combines cosine annealing (2e-3 -> 1e-5) with item + attr L2 (1e-3 each).
# Hypothesis: L2 bounds deviations during the high-LR warmup phase, making the
# aggressive initial LR usable and eliminating the instability seen in cosine-alone.
...
EPOCHS=300
LR=2e-3             # <-- ablated: cosine max LR = 2e-3 (base constant LR = 2e-4)
LR_SCHEDULE="cosine"
...
RUN_NAME="ablation4_cosinebothreg_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
RUN_NAME="${RUN_NAME}_itemdrop${ITEM_DROPOUT_RATE}_lr${LR}to${LR_MIN}_sched${LR_SCHEDULE}"
...
echo ""
echo "============================================================"
echo " Ablation 4: COSINE LR + BOTH L2 REG  (${LR} -> ${LR_MIN})"
echo "  run_name:     $RUN_NAME"
echo "  model:        ${NUM_LAYERS}L ${ATTENTION_HEADS}H emb=${EMBEDDING_DIM} ffn=${NUM_FFN_LAYERS}"
echo "  lr:           ${LR} -> ${LR_MIN}  schedule=${LR_SCHEDULE}"
