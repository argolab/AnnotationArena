...
#SBATCH --job-name=EMF4_BothRegScale
#SBATCH --nodes=1
...
# ── Ablation 4: BOTH L2 REG + SHARED REL SCALE ───────────────────────────────
# In shared-bias mode (--no-per-head-rel), rel_scores are currently added raw
# while content_scores are scaled by 1/sqrt(head_dim). This asymmetry means the
# relational bias is head_dim times larger in magnitude than in per-head mode,
# where both are jointly divided: scores = (content + rel) / sqrt(head_dim).
# --scale-shared-rel applies the missing 1/sqrt(head_dim) factor to rel_scores,
# making the two modes consistent in their relational normalization.
# Combined with both L2 regs to test interaction with the best regularization.
...
SCALE_SHARED_REL=true      # <-- ablated: on (base = off / not passed)
...
ITEM_REG_WEIGHT=1e-3        # <-- same as bothreg winner
ATTRIBUTE_REG_WEIGHT=1e-3   # <-- same as bothreg winner
...
RUN_NAME="ablation4_bothregscale_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
...
RUN_NAME="${RUN_NAME}_relscale_${FLAGS}_${BUNDLE}"

echo ""
echo "============================================================"
echo " Ablation 4: BOTH L2 REG + SHARED REL SCALE (1/sqrt(head_dim))"
...
echo "  init:         ${TYPE_EMBEDDING_INIT}  deviation_norm=false  scale_shared_rel=true"
...
[ "$SCALE_SHARED_REL"   = "true"  ] && SCALE_SHARED_REL_FLAG="--scale-shared-rel"
