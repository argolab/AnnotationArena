...
# ── Ablation 4: ALL THREE — ITEM + ATTR + ANNOTATOR L2 REG ───────────────────
# Extends the bothreg winner by also regularizing annotator deviations (1e-3).
# Tests whether annotator reg helps (hypothesis: it hurts, since annotators are
# transductive — regularizing their biases suppresses useful signal).
...
ANNOTATOR_REG_WEIGHT=1e-3   # <-- ablated: 1e-3 (base = 0.0)
...
RUN_NAME="ablation4_allreg_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
...
echo ""
echo "============================================================"
echo " Ablation 4: ALL THREE REG = item=${ITEM_REG_WEIGHT} / attr=${ATTRIBUTE_REG_WEIGHT} / annot=${ANNOTATOR_REG_WEIGHT}"
