...
#SBATCH --job-name=EMF4_BothRegDevNorm
#SBATCH --nodes=1
...
# ── Ablation 4: BOTH L2 REG + DEVIATION NORM ─────────────────────────────────
# Combines the ablation3 winner (item + attr L2 = 1e-3) with deviation norm.
# Tests whether the two regularization mechanisms are complementary:
# L2 bounds parameter magnitude, deviation norm bounds representational scale.
...
USE_DEVIATION_NORM=true     # <-- ablated: on (base = off)
...
ATTRIBUTE_REG_WEIGHT=1e-3   # <-- ablated: 1e-3 (base = 0.0)
...
RUN_NAME="ablation4_bothregdevnorm_${NUM_LAYERS}L${ATTENTION_HEADS}H_emb${EMBEDDING_DIM}_ffn${NUM_FFN_LAYERS}_${EPOCHS}ep"
...
RUN_NAME="${RUN_NAME}_devnorm_${FLAGS}_${BUNDLE}"

echo ""
echo "============================================================"
echo " Ablation 4: BOTH L2 REG + DEVIATION NORM"
...
echo "  init:         ${TYPE_EMBEDDING_INIT}  deviation_norm=true"
...
[ "$USE_DEVIATION_NORM" = "true"  ] && DEVIATION_NORM_FLAG="--use-deviation-norm"
