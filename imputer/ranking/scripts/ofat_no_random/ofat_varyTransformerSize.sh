#!/bin/bash
# OFAT: Vary transformer size Experiments (small, medium, large)
...
# Transformer architecture - will be varied
# Center (medium) values:
MARFORMER_EMBEDDING_DIM_MED=72
MARFORMER_ENCODER_LAYERS_MED=6
MARFORMER_ATTENTION_HEADS_MED=8
MARFORMER_NUM_FFN_LAYERS_MED=2
...
run_experiment() {
    ...
    local run_name="${OFAT_PREFIX}_varyTransformerSize_${size_name}_D${d_str}_sa${sa_str}_sm${sm_str}_kp${kp_str}_uc${uc_str}_${BASE_PROTOCOL_CODE}"

    echo ""
    echo "=========================================="
    echo "EXPERIMENT: $run_name"
    echo "  Size: $size_name"
    echo "  embedding_dim=$embedding_dim, encoder_layers=$encoder_layers, attention_heads=$attention_heads, num_ffn_layers=$num_ffn_layers"
    echo "=========================================="
    echo ""
...
# Main execution
echo "=============================================="
echo "OFAT: VARY TRANSFORMER SIZE EXPERIMENTS"
echo "=============================================="
echo ""
...
echo ""
echo "Transformer size configurations:"
echo "  Small:  embedding_dim=48,  layers=4, heads=4,  ffn_layers=1"
echo "  Medium: embedding_dim=72,  layers=6, heads=8,  ffn_layers=2 (center)"
echo "  Large:  embedding_dim=128, layers=8, heads=12, ffn_layers=4"
echo ""
echo "Total experiments: 3"
echo "  Vary transformer size: 3 (small, medium, large)"
echo ""
...
# ===== OFAT: Vary transformer size =====
echo "======================================"
echo "OFAT SET: Varying transformer size"
echo "======================================"

# Small
run_experiment "small" 48 2 4 1
if [ $? -ne 0 ]; then
    ((FAILED_EXPERIMENTS++))
fi
((TOTAL_EXPERIMENTS++))
...
# Medium (center)
run_experiment "medium" 72 4 8 2
if [ $? -ne 0 ]; then
    ((FAILED_EXPERIMENTS++))
fi
((TOTAL_EXPERIMENTS++))
...
# Large
run_experiment "large" 128 6 8 2
if [ $? -ne 0 ]; then
    ((FAILED_EXPERIMENTS++))
fi
((TOTAL_EXPERIMENTS++))
...
echo ""
echo "=============================================="
echo "OFAT VARY TRANSFORMER SIZE EXPERIMENTS COMPLETE"
echo "=============================================="
...
echo "  - Data: OUTPUT/generated_data/${OFAT_PREFIX}_varyTransformerSize_*"
echo "  - Marformer: OUTPUT/IMPUTER/${OFAT_PREFIX}_varyTransformerSize_*"
echo "  - Stan (4c): OUTPUT/domain_model/runs/${OFAT_PREFIX}_varyTransformerSize_*_stan4c"
echo "  - Stan (4c) Eval: OUTPUT/domain_model/eval/${OFAT_PREFIX}_varyTransformerSize_*_stan4c_eval"
echo "  - Stan (1c): OUTPUT/domain_model/runs/${OFAT_PREFIX}_varyTransformerSize_*_stan1c"
echo "  - Stan (1c) Eval: OUTPUT/domain_model/eval/${OFAT_PREFIX}_varyTransformerSize_*_stan1c_eval"
echo "=============================================="
