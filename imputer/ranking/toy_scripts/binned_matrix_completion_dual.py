...
"""
Matched binned matrix-completion runner for:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Run binned matrix completion on BERT and EntityMF with matched settings.")
    ...
    p.add_argument("--bin-min", type=float, default=None, help="Optional explicit binning range min; default uses latent-product bounds.")
    p.add_argument("--bin-max", type=float, default=None, help="Optional explicit binning range max; default uses latent-product bounds.")
    ...
    if (args.bin_min is None) != (args.bin_max is None):
        raise ValueError("bin-min and bin-max must be both set or both None")
    if args.bin_min is not None and args.bin_max is not None and args.bin_max <= args.bin_min:
        raise ValueError(f"bin-max must be > bin-min, got [{args.bin_min}, {args.bin_max}]")
    ...
    print(
        ...
        f"binned=True num_bins={args.num_bins} bin_min={args.bin_min} bin_max={args.bin_max} "
    ...
    if not args.emf_only:
        print("Running BERT binned baseline...")
        bert_mod.run_experiment(
            out_dir=out / "bert_aligned_binned",
    ...
    print("Running EntityMF binned model...")
    emf_mod.run(
        out_dir=out / "entity_mf_rowcol_tokens_binned",
