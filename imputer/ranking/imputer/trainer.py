...
def evaluate_with_test_data(self, batch, test_data, converter, verbose=True):
    ...
    with torch.no_grad():
        ...
        for i, var in enumerate(all_variables):
            if var.get('source') == 'test':
                if var['type'] == 'rating' and test_rating_mask[0, i]:
                    ...
                    # Store for Spearman correlation calculation
                    pred_ranking = predictions_full[i]
                    if hasattr(pred_ranking, 'ranking_scores') and pred_ranking.ranking_scores is not None:
                        # Extract predicted scores/rankings
                        pred_scores = pred_ranking.ranking_scores.cpu().numpy()
                        # Only consider valid positions (non-zero ground truth)
        ...
        # Calculate Spearman correlation for rankings
        spearman_rho = None
        spearman_pvalue = None
        ...
        if verbose:
            ...
            if spearman_rho is not None:
                ...
                print(f"Correlation strength: {'Strong' if abs(spearman_rho) > 0.7 else 'Moderate' if abs(spearman_rho) > 0.3 else 'Weak'}")
            else:
                print("Spearman correlation could not be calculated")
