#!/bin/bash
# Single Experiment Runner - LARGE MODEL
# Configure parameters below and run a single experiment with large model architecture
# Data is kept easy for faster convergence

# ============================================
# CONFIGURATION KNOBS - Adjust these values
# ============================================

# Data dimensions (KEPT SMALL/EASY)
BASE_I=3
BASE_J=12
BASE_C=5
BASE_K_TRAIN=10
...
# Data generation hyperparameters (EASY DATA - low noise, high kappa)
BASE_D=16
BASE_SIGMA_ANNOTATOR=0.3  # Lower noise for easier data
BASE_SIGMA_MEASUREMENT=0.05  # Lower noise for easier data
BASE_KAPPA=10  # Higher kappa = sharper rankings = easier
...
# LARGE MODEL ARCHITECTURE PARAMETERS
MARFORMER_ENCODER_LAYERS=12    # Increased from default 6
MARFORMER_ATTENTION_HEADS=8   # Increased from default 8
MARFORMER_EMBEDDING_DIM=512    # Large embedding dimension (much larger than BASE_D * 3 = 48)
MARFORMER_NUM_FFN_LAYERS=2     # Increased from default 4
MARFORMER_DROPOUT=0.1          # Standard dropout
...
RUN_NAME="marformer_debug_norm_fix"  # Leave empty for auto-generated name, or specify custom name
...
if [ -z "$RUN_NAME" ]; then
    ...
    RUN_NAME="single_large_D${d_str}_sa${sa_str}_sm${sm_str}_kp${kp_str}_uc${uc_str}_${BASE_PROTOCOL_CODE}"
fi

echo "=============================================="
echo "SINGLE EXPERIMENT RUNNER - LARGE MODEL"
echo "=============================================="
echo ""
echo "Configuration:"
...
echo ""
echo "Marformer settings (LARGE MODEL):"
echo "  Architecture:"
echo "    Encoder layers: $MARFORMER_ENCODER_LAYERS (default: 6)"
echo "    Attention heads: $MARFORMER_ATTENTION_HEADS (default: 8)"
echo "    Embedding dim: $MARFORMER_EMBEDDING_DIM (default: 128, typical: 48)"
echo "    FFN layers: $MARFORMER_NUM_FFN_LAYERS (default: 4)"
...
# Step 2: Run Marformer (LARGE MODEL)
echo "[Step 2/6] Running Marformer (LARGE MODEL)..."
