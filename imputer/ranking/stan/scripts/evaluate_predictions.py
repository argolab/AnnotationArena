...
def main():
    ...
    try:
        ...
        print(f"\n=== EVALUATION RESULTS ===")
        print(f"\nRating Predictions:")
        print(f"  Accuracy: {results.metrics['rating_accuracy']:.3f}")
        print(f"  MAE: {results.metrics['rating_mae']:.3f}")
        print(f"  Log-likelihood: {results.metrics['rating_log_likelihood']:.3f}")
        print(f"  Calibration error: {results.metrics['rating_calibration_error']:.3f}")
        print(f"  N missing: {results.metrics['n_missing_ratings']}")
        
        print(f"\nPairwise Predictions:")
        print(f"  Accuracy: {results.metrics['pairwise_accuracy']:.3f}")
        print(f"  Log-likelihood: {results.metrics['pairwise_log_likelihood']:.3f}")
        print(f"  AUC: {results.metrics['pairwise_auc']:.3f}")
