data {
    ...
    // Dawid-Skene confusion matrix prior concentration.
    // Diagonal entries of each row's Dirichlet prior = alpha_confusion.
    // Off-diagonal = 1.0. Higher values → confusion matrix closer to identity.
...
generated quantities {
    ...
    // Shared C×C confusion matrix: confusion_matrix[c_lat][c_obs] = P(observe c_obs | latent bin c_lat).
    // Each row is a simplex; generated from Dirichlet(alpha_dir) where alpha_dir[c_lat]=alpha_confusion, others=1.
    ...
    // Posterior rating probabilities (over observed categories, after confusion corruption)
    ...
    // ===== GENERATE DAWID-SKENE CONFUSION MATRIX =====
    // confusion_matrix[c_lat][c_obs] = P(observe c_obs | latent bin = c_lat)
    // Prior: row c ~ Dirichlet(alpha_dir) where alpha_dir[c] = alpha_confusion, alpha_dir[c'] = 1 for c' != c
    {
        for (c in 1:C) {
    ...
    // Dawid-Skene: sample latent bin via ordinal probit, then corrupt through confusion_matrix.
    for (k in 1:K_train) {
        ...
        for (i in 1:I) {
            for (s in 1:num_annotate_annotator) {
                ...
                real cdf_val = Phi(train_base_scores[idx, k] / pref_norm);
                int latent_bin = C;
                for (c in 1:C) {
                ...
                // Corrupt through confusion matrix: observe category drawn from row latent_bin
    ...
    for (k in 1:K_val) {
        ...
        for (i in 1:I) {
            for (s in 1:num_annotate_annotator) {
                ...
                real pref_norm = sqrt(dot_self(annotator_preferences[idx]));
                real cdf_val = Phi(val_base_scores[idx, k] / pref_norm);
                int latent_bin = C;
                for (c in 1:C) {
                    if (cdf_val <= rating_cumprobs[idx][c]) {
                        latent_bin = c;
    ...
    for (k in 1:K_test) {
        ...
        for (i in 1:I) {
            for (s in 1:num_annotate_annotator) {
                ...
                real pref_norm = sqrt(dot_self(annotator_preferences[idx]));
                real cdf_val = Phi(test_base_scores[idx, k] / pref_norm);
                int latent_bin = C;
                for (c in 1:C) {
                    if (cdf_val <= rating_cumprobs[idx][c]) {
                        latent_bin = c;
    ...
    // For Dawid-Skene, the predictive distribution over observed categories is:
    //   P(c_obs | base_score) = sum_{c_lat} P(latent_bin=c_lat | base_score) * confusion_matrix[c_lat][c_obs]
    ...
    for (i in 1:I) {
        for (j in 1:J) {
            ...
            for (k in 1:K_train) {
                real pref_norm = sqrt(dot_self(annotator_preferences[idx]));
                real norm_score = train_base_scores[idx, k] / pref_norm;
                int one_c = C;
                for (c in 1:C) {
                    if (norm_score <= rating_thresholds_z[idx][c+1]) {
                        one_c = c;
                        break;
                    }
                }
                for (c in 1:C) {
                    train_posterior_rating_probs[idx, k][c] = confusion_matrix[one_c][c];
                }
            }
        }
    }
    ...
    for (i in 1:I) {
        for (j in 1:J) {
            ...
            for (k in 1:K_val) {
                real pref_norm = sqrt(dot_self(annotator_preferences[idx]));
                real norm_score = val_base_scores[idx, k] / pref_norm;
                int one_c = C;
                for (c in 1:C) {
                    if (norm_score <= rating_thresholds_z[idx][c+1]) {
                        one_c = c;
                ...
                for (c in 1:C) {
                    val_posterior_rating_probs[idx, k][c] = confusion_matrix[one_c][c];
                }
            }
        }
    }
    ...
    for (i in 1:I) {
        for (j in 1:J) {
            ...
            for (k in 1:K_test) {
                real pref_norm = sqrt(dot_self(annotator_preferences[idx]));
                real norm_score = test_base_scores[idx, k] / pref_norm;
                int one_c = C;
                for (c in 1:C) {
                    if (norm_score <= rating_thresholds_z[idx][c+1]) {
                        one_c = c;
                ...
                for (c in 1:C) {
                    test_posterior_rating_probs[idx, k][c] = confusion_matrix[one_c][c];
                }
            }
        }
    }
    ...
    if (DEBUG_PRINT == 1) {
        print("kappa=", kappa);
        print("alpha_confusion=", alpha_confusion);
        print("confusion_matrix[1]=", confusion_matrix[1]);
