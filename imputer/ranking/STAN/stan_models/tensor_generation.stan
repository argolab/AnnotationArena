 ...
 *
 * Threshold model (derive_thresholds_from_annotator):
 *   0 = independent Dirichlet per (i,j): rating_probs[ij] ~ Dirichlet(kappa/C, ...)
 *   1 = linear map from alpha_j: threshold_logits = W * alpha_j + bias_i
 *       where W is (C-1) x T, bias_i is (C-1)-dim attribute bias
 */
...
generated quantities {
    ...
    if (derive_thresholds_from_annotator == 1) {
        // threshold_logits = W * alpha_j + bias_i   (C-1 dim)
        for (i in 1:I) {
            for (j in 1:J) {
                int idx = (i-1)*J + j;
                vector[C-1] threshold_logits = threshold_transform_W * alpha_jt[j]
                                               + to_vector(threshold_attr_bias[i]);
                // Convert logits to simplex via softmax with a pinned zero for category 1
    ...
    // Convert cumulative probs to standard-normal z-space thresholds
    for (ij in 1:(I*J)) {
        ...
        for (j in 1:J) {
            ...
            for (k in 1:K_train) {
                ...
                if (use_dawid_skene_noise == 1) {
                    ...
                    for (c in 1:C) {
                        real p_upper = (rating_thresholds_z[idx][c+1] == positive_infinity()) ? 1.0
                                     : Phi((rating_thresholds_z[idx][c+1] - z_std) / sigma_std);
                        real p_lower = (rating_thresholds_z[idx][c] == negative_infinity()) ? 0.0
                                     : Phi((rating_thresholds_z[idx][c] - z_std) / sigma_std);
                        train_posterior_rating_probs[idx, k][c] = p_upper - p_lower;
                    }
                }
            }
            ...
            for (k in 1:K_val) {
                ...
                if (use_dawid_skene_noise == 1) {
                    ...
                    for (c in 1:C) {
                        ...
                        real p_lower = (rating_thresholds_z[idx][c] == negative_infinity()) ? 0.0
                                     : Phi((rating_thresholds_z[idx][c] - z_std) / sigma_std);
                        val_posterior_rating_probs[idx, k][c] = p_upper - p_lower;
                    }
                }
            }
            ...
            for (k in 1:K_test) {
                ...
                if (use_dawid_skene_noise == 1) {
                    ...
                    for (c in 1:C) {
                        ...
                        real p_lower = (rating_thresholds_z[idx][c] == negative_infinity()) ? 0.0
                                     : Phi((rating_thresholds_z[idx][c] - z_std) / sigma_std);
                        test_posterior_rating_probs[idx, k][c] = p_upper - p_lower;
                    }
                }
            }
        }
