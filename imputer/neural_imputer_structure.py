"""
Neural imputer implementation for graph imputation (structure-only version).

This module contains the transformer-based neural imputer model architecture 
that uses only graph structure (adjacency matrix) and observed node states.
...
class GraphImputer(nn.Module):
    """Main model for graph imputation using transformer."""
    ...
    def __init__(self, 
        ...
        print(f"GraphImputer (structure-only) initialized: {n_nodes} nodes, {n_states} states per node")
...
def evaluate_neural_model(model, test_data, n_nodes, n_states=2):
    """Evaluate neural model and compute KL divergence."""
    print(f"Evaluating neural model (structure-only) on {len(test_data)} test samples...")
