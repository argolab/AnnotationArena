#!/usr/bin/env python3
"""
Mechanistic Interpretability (MI) experiments for MARFORMER.

Runs experiments at specific training budgets [50, 500, 2000] with layer-wise
analysis to understand how transformer layers progressively refine predictions.

This script is designed to analyze whether MARFORMER learns belief propagation-like
inference algorithms through mechanistic interpretability techniques (logit lens).
"""
...
from experiments.mi_experiment_runner import run_mi_experiment_suite
...
def setup_logging(output_dir: Path, log_level: str = "INFO") -> None:
    """
    Setup logging for MI experiments.
    ...
    file_handler = logging.FileHandler(logs_dir / "mi_experiment.log")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Mechanistic Interpretability Analysis for MARFORMER",
    ...
    parser.add_argument('--budgets', type=int, nargs='+', default=[50, 500, 2000],
                       help='Specific training budgets for MI analysis')
    ...
    parser.add_argument('--output-dir', type=Path, default=Path('./OUTPUT_MI'),
...
def print_experiment_summary(args: argparse.Namespace) -> None:
    ...
    print("=" * 80)
    print("MECHANISTIC INTERPRETABILITY EXPERIMENT CONFIGURATION")
    print("=" * 80)
...
def main() -> None:
    """Main entry point for MI experiments."""
    try:
        # Parse arguments
        ...
        logger.info("Starting Mechanistic Interpretability experiments...")

        # Create MI policy with specific budgets
        ...
        results = run_mi_experiment_suite(
        ...
        print()
        print("=" * 80)
        print("MI EXPERIMENTS COMPLETED SUCCESSFULLY")
        print("=" * 80)
        ...
        print(f"  - Layer-wise KL data:  {args.output_dir}/*/budget_*/*/layer_analysis.json")
        print(f"  - Raw KL data:         {args.output_dir}/*/budget_*/*/layer_kl_raw_data.pkl")
        print(f"  - Trained models:      {args.output_dir}/*/budget_*/*/model.pt")
        print(f"  - Logs:                {args.output_dir}/logs/")
        print("=" * 80)

        logger.info("MI experiments completed successfully")
