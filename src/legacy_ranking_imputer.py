                ...
                if var_type == 0:  # Rating variable
                    ...
                    if len(valid_items) > 1:
                        ...
                        if len(item_attr_embeddings) > 1:
                            # Compute consecutive outer products
                            current_embed = item_attr_embeddings[0]
                            for i in range(1, len(item_attr_embeddings)):
                                outer_product = torch.outer(current_embed, item_attr_embeddings[i])
                                current_embed = self.ranking_projection(outer_product.flatten())
                            feature_embeddings[b, v] = current_embed
                        else:
                            feature_embeddings[b, v] = attr_embeds[b, v] + annot_embeds[b, v]
                    else:
                        feature_embeddings[b, v] = attr_embeds[b, v] + annot_embeds[b, v]
...
class DataConverter:
    ...
    def load_training_data(self, json_file):
        ...
        for ranking in data['rankings']:
            ...
            if all(item <= self.num_items for item in ranking['items'][:3]):
    ...
    def create_all_possible_variables(self):
        """Create all possible rating and ranking variable combinations"""
        rating_variables = []
        ranking_variables = []
        
        # All possible rating variables: (attribute, annotator, item)
        for attr in range(self.num_attributes):
            for annot in range(self.num_annotators):
                for item in range(1, self.num_items + 1):  # Items are 1-indexed
        ...
        # All possible ranking variables: (attribute, annotator, item_subset)
        # Using combinations of size max_rank_size
        for attr in range(self.num_attributes):
            for annot in range(self.num_annotators):
                for item_combo in combinations(range(1, self.num_items + 1), self.max_rank_size):
                    ranking_variables.append({
                        'type': 'ranking',
                        'attribute': attr,
                        'annotator': annot,
                        'items': list(item_combo)
                    })
    ...
    def process_training_data(self, data):
        ...
        for ranking in data['rankings']:
            ...
            key = (ranking['attribute'], ranking['annotator'], tuple(sorted(items)))
            ranking_data[key] = {
                'items': items,
                'order': order
            }
    ...
    def create_training_batch(self, rating_variables, ranking_variables, 
                            rating_data, ranking_data, test_data=None):
        ...
        for i, var in enumerate(all_variables):
            attribute_ids[0, i] = var['attribute']
            annotator_ids[0, i] = var['annotator']
            ...
            if var['type'] == 'rating':
                ...
                if key in rating_data and key not in test_exclusions:
                    ...
                    # Also set the variable_data for input
                ...
                key = (var['attribute'], var['annotator'], tuple(sorted(items)))
...
class ImputerTrainer:
    ...
    def train_step(self, batch):
        ...
        rating_loss = 0.0
        ranking_loss = 0.0
        print(rating_logits.shape)
        print(ranking_logits.shape)
        ...
        if rating_mask.any():
            ...
            if len(rating_indices) > 0:
                ...
                rating_loss = self.rating_loss_fn(valid_rating_logits, target_classes).mean()
        ...
        return {
            ...
            'rating_loss': rating_loss.item() if isinstance(rating_loss, torch.Tensor) else rating_loss,
            'ranking_loss': ranking_loss.item() if isinstance(ranking_loss, torch.Tensor) else ranking_loss
        }
    
    def evaluate_with_test_data(self, batch, test_data, converter):
...
def main():
    ...
    rating_variables, ranking_variables = converter.create_all_possible_variables()
