...
class DomainSpecificModel(nn.Module):
    ...
    def __init__(self, n: int, k: int, sparsity_pattern: np.ndarray, 
        ...
        # Extract initialization values from metadata if provided
        if init_metadata is not None:
            # Extract Omega matrix from metadata
            if 'true_parameters' in init_metadata and 'Omega' in init_metadata['true_parameters']:
                init_omega = np.array(init_metadata['true_parameters']['Omega'])
                # Initialize from provided precision matrix
                init_omega_tensor = torch.tensor(init_omega, dtype=torch.float32)
                L_init = torch.linalg.cholesky(init_omega_tensor)
                self.L_lower = nn.Parameter(torch.tril(L_init, diagonal=-1))
                self.L_diag_log = nn.Parameter(torch.log(torch.diag(L_init)))
                
                # Compute p_mat from boundaries and omega (reverse process)
                if 'boundaries' in init_metadata:
                    boundaries = np.array(init_metadata['boundaries'])
                    sigma = np.linalg.inv(init_omega)  # Covariance matrix
                    
                    # Compute probability matrix from boundaries
                    init_p_mat = np.zeros((n, k))
                    for i in range(n):
                        sigma_i = np.sqrt(sigma[i, i])  # Standard deviation for variable i
                        
                        # Convert thresholds back to cumulative probabilities
                        cumulative_probs = []
                        for j in range(k-1):
                            # Reverse: threshold = sigma_i * erfinv(2 * cumsum_p - 1) * sqrt(2)
                            # So: cumsum_p = 0.5 * (1 + erf(threshold / (sigma_i * sqrt(2))))
                            threshold = boundaries[i, j]
                            cumsum_p = 0.5 * (1 + torch.erf(torch.tensor(threshold / (sigma_i * np.sqrt(2)))))
                            cumulative_probs.append(cumsum_p.item())
                        
                        # Convert cumulative probabilities to individual probabilities
                        prev_cumsum = 0.0
                        for j in range(k):
                            if j < k-1:
                                current_cumsum = cumulative_probs[j]
                                init_p_mat[i, j] = current_cumsum - prev_cumsum
                                prev_cumsum = current_cumsum
                            else:
                                # Last category gets remaining probability
                                init_p_mat[i, j] = 1.0 - prev_cumsum
                    
                    # Ensure probabilities are valid (positive and sum to 1)
                    init_p_mat = np.maximum(init_p_mat, 1e-8)  # Ensure positive
                    init_p_mat = init_p_mat / init_p_mat.sum(axis=1, keepdims=True)  # Normalize
                    
                    # Initialize from computed probability matrix
                    init_p_tensor = torch.tensor(init_p_mat, dtype=torch.float32)
                    # Convert probabilities to logits (inverse softmax)
                    init_logits = torch.log(init_p_tensor + 1e-8)
                    self.p_logits = nn.Parameter(init_logits)
                else:
                    # No boundaries available, use random initialization
                    self.p_logits = nn.Parameter(torch.randn(n, k) * 0.1)
            else:
                # Default random initialization for precision matrix
                self.L_lower = nn.Parameter(torch.randn(n, n) * 0.1)
                self.L_diag_log = nn.Parameter(torch.ones(n))
                self.p_logits = nn.Parameter(torch.randn(n, k) * 0.1)
        else:
            # Default random initialization when no metadata provided
            self.L_lower = nn.Parameter(torch.randn(n, n) * 0.1)
            self.L_diag_log = nn.Parameter(torch.ones(n))
            self.p_logits = nn.Parameter(torch.randn(n, k) * 0.1)
...
class GaussianBinningWithLinGauss:
    ...
    def fit_map_with_pytorch(self, x_obs_batch: np.ndarray, obs_idx_batch: List[np.ndarray],
                            ...
                            lambda_sparsity: float = 100.0, device: str = 'cpu', init_metadata=None) -> Dict:
        """
        ...
        model = DomainSpecificModel(
            ...
            alpha=self.alpha, nu=self.nu, V=self.V, init_metadata=init_metadata
        ).to(device)
    ...
    def export_training_data(self, data: Dict, missing_prob: float = 0.3, 
        ...
        metadata = {
            ...
            "sparsity_pattern": data["sparsity_pattern"].tolist(),
...
def generate_training_data(n: int = 10, k: int = 4, num_datapoints: int = 1000,
                          missing_prob: float = 0.3, output_prefix: str = "gaussian",
    ...
    # Generate data
    data = model.generate_synthetic_data(seed=42, num=num_datapoints)
    ...
    # Export train data
    train_file = f"{output_prefix}_train_{n}_new.json"
    ...
    # Export dev data
    dev_file = f"{output_prefix}_dev_{n}_new.json"
    ...
    print(f"  Training: {train_file} ({split_idx} examples)")
    print(f"  Dev: {dev_file} ({num_datapoints - split_idx} examples)")
